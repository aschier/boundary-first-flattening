...
typedef Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> cholmod_dense;

//extern Common common;

inline cholmod_dense* cholmod_l_zeros(size_t m, size_t n)
{
	return new cholmod_dense(cholmod_dense::Zero(m, n));
}
inline cholmod_dense* cholmod_l_eye(size_t m, size_t n)
{
	return new cholmod_dense(cholmod_dense::Identity(m, n));
}
inline cholmod_dense* cholmod_l_ones(size_t m, size_t n)
{
	return new cholmod_dense(cholmod_dense::Ones(m, n));
}
inline cholmod_dense* cholmod_l_copy_dense(cholmod_dense* input)
{
...
{
    return DenseMatrix(cholmod_l_eye(m, n));
}

inline DenseMatrix DenseMatrix::ones(size_t m, size_t n)
{
    return DenseMatrix(cholmod_l_ones(m, n));
}
...
{
	return data->norm(); // cholmod_l_norm_dense(data, norm, common);
}
...
inline DenseMatrix DenseMatrix::submatrix(const std::vector<int>& r, const std::vector<int>& c) const
{
    size_t m = r.size();
    size_t n = c.size();
    DenseMatrix A(m, n);
...
{
    return cholmod_l_copy_dense(data);
}
...
{
	return DenseMatrix(new cholmod_dense((*A.data) * s));
...
{
	return DenseMatrix(new cholmod_dense((*A.data) + (*B.data)));
    /*DenseMatrix C(B);
...
{
	return data->coeffRef(r, c); // ((double *)data->x)[c*nRows() + r];
}
